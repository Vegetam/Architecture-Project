...
test('GET /health returns 200 with status ok', async () => {
  ...
  assert.equal(r.body.service, 'payment-service');
});

test('POST /payments returns 200 with payment id', async () => {
  const r = await request('POST', '/payments', { orderId: 'order-1', amount: 99.99 });
  assert.equal(r.status, 200);
  assert.ok(r.body.id, 'response must include a payment id');
